...
def generate_url(period: Period, pair: str, pair_type: PairType, date: datetime|str, futures_type: Optional[FuturesType]) -> str:
    ...
    date_str = date.strftime("%Y-%m-%d") if period == Period.DAILY else date.strftime("%Y-%m")
...
def download_file(url: str, output_path: str) -> None:
    ...
    if response.status_code != 200:
        raise ValueError('Error downloading file, status code: ', response.status_code)
...
if __name__ == '__main__':
    ...
    parser.add_argument('period', help='data period', choices=['daily', 'monthly'])
    parser.add_argument('-f', '--futures', help='download futures data', action='store_true')
    parser.add_argument('-t', '--type', help='futures type', choices=['cm', 'um'], required=False)
